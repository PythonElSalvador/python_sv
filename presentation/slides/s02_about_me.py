...
def about_me_slide() -> RenderableType:
    ...
    content.append(
        "· Construyendo herramientas de IA para transformar\n", style=f"{TEXT}"
    )
    content.append("  cómo se escribe y optimiza el código\n\n", style=f"{TEXT}")
    content.append("· Arquitectura de software, dev tools, IA\n", style=f"{TEXT}")
    content.append(
        "  y hacer la tecnología más accesible para todos\n\n", style=f"{TEXT}"
    )
    content.append("· Speaker en PyCon US y PyLatam\n\n", style=f"{TEXT}")
    content.append(
        "· Parte del grupo de trabajo de Becas de la PSF,\n", style=f"{TEXT}"
    )
    content.append(
        "  donde ayudo a hacer que las comunidades de Python\n", style=f"{TEXT}"
    )
