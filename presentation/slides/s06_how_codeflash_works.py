...
def how_codeflash_works_slide() -> RenderableType:
    header = Text(figlet_format("Como funciona?", font="small"), style=f"bold {ACCENT}")

    steps = Text()
    steps.append("  1. ", style=f"bold {FLASH}")
    steps.append("Analiza tu código\n", style=f"bold {TEXT}")
    steps.append(
        "     Identifica funciones con potencial de optimización\n\n", style=f"{MUTED}"
    )

    steps.append("  2. ", style=f"bold {FLASH}")
    steps.append("Genera optimizaciones con IA\n", style=f"bold {TEXT}")
    steps.append(
        "     Múltiples estrategias: algoritmos, built-ins, estructuras\n\n",
        style=f"{MUTED}",
    )
